...
%代价函数中，k的累加可以用向量内积实现，扩展到矩阵（样本）就是矩阵乘法，而i的累加是mxm阶矩阵的迹
temp1=temp_y*log(a3)';
...
%calculate gradient
%%无论是价值函数还是求解梯度，都要把所有样本训练结果相加。
%%delta只进行到第一个隐藏层，但Δ除了输出层都要进行，用于更新每层的Θ矩阵
%%除了输出层的δ，其余层由于sigmoid（z）的结果没有加上偏执项，所以θ（l）需要去掉第一列元素，但不意味着梯度结果不包含θ0
